...
class TestHyundaiPcmCruiseEvents:
  ...
  def test_set_waits_for_stock_pcm_rising_edge(self):
    disabled = car_state()
  ...
  def test_resume_from_disabled_pcm_allows_real_rising_edge(self):
    self.cc.enabled = True
    enabled = car_state(cruise_enabled=True)
  ...
  def test_cancel_button_resumes_when_already_disengaged(self):
    self.events.hyundai_cancel_latched = True
    disabled = car_state()
    events = self.events.update(car_state(button_type=ButtonType.cancel), disabled, self.cc)
  ...
  def test_cancel_button_resume_with_same_frame_pcm_rising(self):
    self.events.hyundai_cancel_latched = True
    disabled = car_state()
    events = self.events.update(car_state(cruise_enabled=True, button_type=ButtonType.cancel), disabled, self.cc)
    assert EventName.buttonCancel not in events.names
    assert EventName.pcmEnable in events.names
  ...
  def test_set_cancel_resume_state_machine_sequence(self):
    ...
    self.cc.enabled = False
    events = self.events.update(car_state(button_type=ButtonType.accelCruise), disabled, self.cc)
    assert state_machine.update(events) == (False, False)
